...
uint8_t identify() {
    ...
    io_wait(); io_wait(); io_wait(); io_wait(); // 400ns delay required by spec
    ...
    if (status == 0 || status == 0xFF) {
        printf("No drive detected (status = 0x%02X)\n", status);
        return 0;
    }

    // Wait for BSY to clear
    int timeout = 1000000;
    while ((status = inb(ATA_PRIMARY_COMM_REGSTAT)) & STAT_BSY && --timeout);
    if (timeout == 0) {
        PANIC("IDENTIFY: BSY never cleared");
        return 0;
    }

    // Check for non-ATA device (like ATAPI)
    uint8_t mid = inb(ATA_PRIMARY_LBA_MID);
    uint8_t hi  = inb(ATA_PRIMARY_LBA_HI);
    if (mid != 0 || hi != 0) {
        printf("Not an ATA device (LBAMID = 0x%02X, LBAHI = 0x%02X)\n", mid, hi);
        return 0;
    }

    // Wait for DRQ or ERR
    timeout = 1000000;
    while (!((status = inb(ATA_PRIMARY_COMM_REGSTAT)) & (STAT_DRQ | STAT_ERR)) && --timeout);
    if (timeout == 0 || (status & STAT_ERR)) {
        PANIC("IDENTIFY: ERR or timeout while waiting for DRQ");
        return 0;
    }

    // Read IDENTIFY data
    uint16_t id_data[256];
    insw(ATA_PRIMARY_DATA, id_data, 256);

    // Parse model string (word 27–46)
    char model[41];
    for (int i = 0; i < 20; i++) {
        model[i * 2 + 0] = (id_data[27 + i] >> 8) & 0xFF;
        model[i * 2 + 1] = (id_data[27 + i] >> 0) & 0xFF;
    }
    model[40] = '\0';
    ...
    // Read disk size (in sectors)
    uint32_t sectors = id_data[60] | ((uint32_t)id_data[61] << 16);
    uint32_t size_mib = sectors / 2048;
